...
# Map filename keyword → list of sign names to generate.
# arrow_left  covers both left (-2) and sharp-left (-3) — no dedicated sharp image.
# arrow_right covers both right (+2) and sharp-right (+3).
# u_turn_left produces the U-turn bitmap (sign_m4, -4).
SIGN_MAP = {
    'straight':         ['sign_0'],
    'arrow_left':       ['sign_m2', 'sign_m3'],
    'arrow_right':      ['sign_2',  'sign_3'],
    'slight_left':      ['sign_m1'],
    'slight_right':     ['sign_1'],
    'u_turn_left':      ['sign_m4'],
    'check_circle':     ['sign_4'],
    'roundabout_left':  ['sign_6'],
    'roundabout_right': ['sign_6'],
}
...
def match_key(filename):
    ...
    for keyword, signs in SIGN_MAP.items():
        if keyword in name:
            return signs
    return None
